...
export class MfacultyComponent implements OnInit {
  ...
  fetchstan() {
    this.http.get("http://localhost:8050/Admin/Faculty").subscribe((data: any) => {
      this.tbldata = data
      var that = this
          ...
          Is_Active: {
            ...
            onComponentInitFunction(instance) {
              instance.save.subscribe((data: any) => {
                that.http.put("http://localhost:8050/Admin/Faculty", { _id: data.rowdata._id, Is_Active: data.value }).subscribe((data: any) => {
                  console.log(data)
                  if (data.Error) {
  ...
  onDeleteConfirm(event) {
    ...
    swal.fire({
      ...
      if (data.dismiss === swal.DismissReason.cancel) {
        ...
        that.http.delete("http://localhost:8050/Admin/Faculty?_id=" + event.data._id).subscribe((data: any) => {
          if (data.Error) {
            that.toast.error(data.Error);
          } else {
