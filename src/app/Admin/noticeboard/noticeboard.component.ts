...
export class NoticeBoardComponent implements OnInit {
  ...
  handledelete(id) {
    ...
    swal.fire({
      ...
      if (data.dismiss === swal.DismissReason.cancel) {
        ...
        that.http.delete("https://college-managment-system.herokuapp.com/Admin/Notice_Borad?_id=" + id).subscribe((data: any) => {
          if (data.Error) {
  ...
  submitForm() {
    if (this.isupdate) {
      this.isupdate = false
      this.http.put("https://college-managment-system.herokuapp.com/Admin/Notice_Borad", { ...this.myForm.value, _id: this.id }).subscribe((data: any) => {
        if (data.Error) {
      ...
      this.http.post("https://college-managment-system.herokuapp.com/Admin/Notice_Borad", this.myForm.value).subscribe((data: any) => {
        if (data.Success) {
          this.toast.success(data.Success);
          this.modalService.dismissAll()
          this.myForm.reset()
